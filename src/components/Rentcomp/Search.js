...
function Search() {
  ...
  const { isUsersearching, searchdata } = useSelector((store) => store.rental)
  ...
  const onSubmit = (values, onSubmitProps) => {
    ...
    const serializedDate = values.date.toISOString()
    dispatch(
      setsearcheddata({
        city: values.city,
        state: values.state,
  ...
  return (
    <>
      <div className=' px-6  '>
        {/* form */}
        <div className='px-4 pt-2 pb-4 md:py-5 md:px-6   bg-white max-w-md sm:w-[34rem] sm:max-w-full md:w-[44rem] md:max-w-full  lg:w-[56rem] xl:w-[64rem]'>
          {/* form */}
          <Formik
